...
print(f'next_checkpoint : {next_checkpoint_x} {next_checkpoint_y}', file=sys.stderr)
print(f'next_checkpoint_dist : {next_checkpoint_dist}', file=sys.stderr)
print(f'next_checkpoint_angle : {next_checkpoint_angle}', file=sys.stderr)
startAngle = getStartAngle(x, y, next_checkpoint_x, next_checkpoint_y)
startAngleOpponent = getStartAngle(opponent_x, opponent_y, next_checkpoint_x, next_checkpoint_y)
print(f'angle de départ : {startAngle} {startAngleOpponent}', file=sys.stderr)

# Create first checkpoint
firstCheckpoint = Checkpoint(next_checkpoint_x, next_checkpoint_y)
next_checkpoint = firstCheckpoint

# Create pods
myPod = Pod(x, y, startAngle)
opponentPod = Pod(opponent_x, opponent_y, startAngleOpponent)
print(myPod.__dict__, file=sys.stderr)

test([myPod, opponentPod], Checkpoint.all, 650)
print(myPod.__dict__, file=sys.stderr)
for i in range(10):
    test([myPod, opponentPod], Checkpoint.all, 100)
    print(myPod.__dict__, file=sys.stderr)
...
while True:
    ...
    print(f'next_checkpoint : {next_checkpoint_x} {next_checkpoint_y}', file=sys.stderr)
    print(f'next_checkpoint_angle : {next_checkpoint_angle}', file=sys.stderr)
    print(Checkpoint.all, file=sys.stderr)
    # Get current checkpoint or create another
    if (next_checkpoint_x, next_checkpoint_y) != (next_checkpoint.x, next_checkpoint.y):
        next_checkpoint = Checkpoint(next_checkpoint_x, next_checkpoint_y)
